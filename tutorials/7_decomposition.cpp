...
int main() {
  std::cout << "####### Tutorial 7.1 Why and when decompose ######"
            << std::endl;
  ...
  F2CSwathsByCells swaths_wo_decomp =
      bf.generateBestSwaths(obj, r_w, no_hl_wo_decomp);
  std::cout << "Without decomposition >> " << obj.computeCost(swaths_wo_decomp)
            << std::endl;
  ...
  f2c::Visualizer::plot(swaths_decomp3);
  f2c::Visualizer::plot(swaths_decomp4);
  f2c::Visualizer::xlim(-5, 95);
  f2c::Visualizer::ylim(-5, 95);
  f2c::Visualizer::show();
  f2c::Visualizer::save("Tutorial_7_1_with_decomposition.png");

  std::cout << "####### Tutorial 7.2 Decomposition with route planner ######"
            << std::endl;
