...
F2CGraph2D RoutePlannerBase::createShortestGraph(const F2CCells& cells,
  ...
  // Add start and end point if they exist.
  // Skip adding the edge when the point is already on the border
  // (distance <= d_tol): closestPointOnBorderTo would return the same
  // point, producing a zero-length self-loop that corrupts the graph
  // (see .planning/research/route-planner-crash.md, Fix 2).
  if (this->r_start) {
    auto border_p_start = cells.closestPointOnBorderTo(*r_start);
    if (border_p_start.distance(*r_start) > d_tol) {
      g.addEdge(*r_start, border_p_start);
    }
  }
  if (this->r_end) {
    auto border_p_end = cells.closestPointOnBorderTo(*r_end);
    if (border_p_end.distance(*r_end) > d_tol) {
      g.addEdge(*r_end, border_p_end);
    }
  }
...
std::vector<long long int> RoutePlannerBase::computeBestRoute(
  ...
  const int transit_callback_index = routing.RegisterTransitCallback(
      [&cov_graph, &manager](long long int from, long long int to)
          -> long long int {
  ...
  if (solution == nullptr) {
    throw std::runtime_error(
        "RoutePlannerBase: OR-Tools found no feasible solution. "
        "Check that start/end points are reachable from swath endpoints.");
  }
...
F2CRoute RoutePlannerBase::transformSolutionToRoute(
  ...
  for (size_t i = 0; i + 2 < route_ids.size(); ++i) {
    ...
    for (int j = 0; j < NS; ++j) {
      ...
      if (p_e == swath.startPoint() && p_s == swath.endPoint()) {
        swath.reverse();
        if (route.isEmpty() && r_start) {
          route.addConnection(
              shortest_graph.shortestPath(*r_start, swath.startPoint()));
