...
F2CField Parser::importFieldGml(
  ...
  const auto* e_result = p_parcel->Attribute("id");
  if (e_result != nullptr) {
    id = e_result;
  }
...
F2CCell getCellFromJson(const json& imported_cell) {
  ...
  // T-025: guard against malformed GeoJSON missing geometry.coordinates.
  // Without these checks, nlohmann::json::operator[] on a non-existent key
  // default-constructs a null value in a non-const context, and subsequent
  // access crashes (heap-use-after-free observed via fuzzing, C-001).
  if (!imported_cell.is_object() || !imported_cell.contains("geometry") ||
      !imported_cell["geometry"].is_object() ||
      !imported_cell["geometry"].contains("coordinates") ||
      !imported_cell["geometry"]["coordinates"].is_array()) {
    throw std::invalid_argument(
        "GeoJSON feature missing geometry.coordinates");
  }
  F2CCell cell;
  const auto& json_rings = imported_cell["geometry"]["coordinates"];
  for (size_t i = 0; i < json_rings.size(); ++i) {
